...
def sliceanddice(SampleArray, FractionsArray, testsplit, Nbags, EventsBag, backend, img_rows, img_cols):

	TrainList = []
	TrainVals = []
	TestList = []
	TestVals = []
	SampleArray = np.reshape(SampleArray, (
	SampleArray.shape[0], SampleArray.shape[1], SampleArray.shape[2], SampleArray.shape[3], -1))
	for i in range(0, Nbags):
		om = slice(0, int(testsplit * EventsBag))
		mo = slice(int((testsplit * EventsBag)), EventsBag)

		pTrain = SampleArray[i:i + 1, om, :, :, :]
		pTest = SampleArray[i:i + 1, mo, :, :, :]

		fTrain = FractionsArray[i:i + 1, om]
		fTest = FractionsArray[i:i + 1, mo]

		TrainList.extend(pTrain)
		TestList.extend(pTest)

		TrainVals.extend(fTrain)
		TestVals.extend(fTest)

	trainsize = sum([X.shape[0] for X in TrainList])
	valsize = sum([X.shape[0] for X in TestList])
	input_shape = (25, 25, 1)

	#x = np.array(TrainList)
	#print x.shape, 
	return TrainList, TestList, TrainVals, TestVals,input_shape, trainsize, valsize
